...
### 3-5. ZaimRowConverterFactory クラスの実装
...
### 5. 口座に依存する属性の定義に対するテストの修正

手順 4. で追加した口座に依存する属性の定義を追加すると、
テストが失敗するようになるので修正します。

### 5-1. `FilePathConvertTable` の修正

`tests/test_account.py` の `FilePathConvertTable` に
追加した口座のお店、品目の変換テーブルが定義されている
CSV ファイルへのパスの例を追加します。

### 5-2. `FilePathInput` の修正
