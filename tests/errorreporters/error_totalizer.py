...
class TestErrorTotalizer:
    ...
    def test_fail(resource_path: Path, tmp_path: Path) -> None:
        ...
        files = sorted(tmp_path.rglob("*[!.gitkeep]"))
        assert len(files) == expected_length, f"files = {files}"
        ...
        with files[1].open("r", encoding="UTF-8", newline="\n") as file_error:
            ...
            assert not error_row_data.store_name
            assert error_row_data.item_name == "".join(
                [
            ...
            assert not error_row_data.item_name
