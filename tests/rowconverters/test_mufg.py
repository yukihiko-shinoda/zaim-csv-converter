...
from zaimcsvconverter.inputcsvformats.mufg import MufgRowFactory, MufgRowData, MufgRow
...
class TestMufgZaimIncomeRowConverter:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (InstanceResource.ROW_DATA_MUFG_INCOME_NOT_CARD,
             '2018-10-01', 'フリコミモト－アカウント', 'お財布', '三菱UFJ銀行', 10000),
            ...
            (InstanceResource.ROW_DATA_MUFG_TRANSFER_INCOME_NOT_OWN_ACCOUNT,
             '2018-08-20', '三菱UFJ銀行', None, '三菱UFJ銀行', 20),
        ]
    )
    def test(mufg_row_data: MufgRowData, expected_date, expected_store, config_transfer_account_name,
        ...
        mufg_row = MufgRow(AccountId.MUFG, mufg_row_data)
        validated_input_row = mufg_row.validate()
        zaim_row = MufgZaimIncomeRowConverter(validated_input_row).convert()
...
class TestMufgZaimPaymentRowConverter:
    ...
    def test(yaml_config_load, database_session_stores_mufg):
        ...
        mufg_row = MufgRow(AccountId.MUFG, InstanceResource.ROW_DATA_MUFG_TRANSFER_PAYMENT_TOKYO_WATERWORKS)
        validated_input_row = mufg_row.validate()
        zaim_row = MufgZaimPaymentRowConverter(validated_input_row).convert()
...
class TestMufgZaimTransferRowConverter:
    ...
    def test_input_row(mufg_row_data, expected_date, config_transfer_account_name, config_account_name, expected_amount,
        ...
        mufg_row = MufgRow(AccountId.MUFG, mufg_row_data)
        validated_input_row = mufg_row.validate()
        zaim_row = MufgIncomeZaimTransferRowConverter(validated_input_row).convert()
    ...
    def test_payment_row(yaml_config_load, database_session_stores_mufg):
        ...
        mufg_row = MufgRow(AccountId.MUFG, InstanceResource.ROW_DATA_MUFG_PAYMENT)
        validated_input_row = mufg_row.validate()
        zaim_row = MufgPaymentZaimTransferRowConverter(validated_input_row).convert()
        assert isinstance(zaim_row, ZaimTransferRow)
        list_zaim_row = zaim_row.convert_to_list()
        zaim_row_data = ZaimRowData(*list_zaim_row)
    ...
    def test_transfer_income_row(yaml_config_load, database_session_stores_mufg):
        ...
        validated_input_row = mufg_row.validate()
        zaim_row = MufgTransferIncomeZaimTransferRowConverter(validated_input_row).convert()
        assert isinstance(zaim_row, ZaimTransferRow)
        list_zaim_row = zaim_row.convert_to_list()
        zaim_row_data = ZaimRowData(*list_zaim_row)
    ...
    def test_transfer_payment_row(yaml_config_load, database_session_stores_mufg):
        ...
        validated_input_row = mufg_row.validate()
        zaim_row = MufgTransferPaymentZaimTransferRowConverter(validated_input_row).convert()
        assert isinstance(zaim_row, ZaimTransferRow)
        list_zaim_row = zaim_row.convert_to_list()
        zaim_row_data = ZaimRowData(*list_zaim_row)
        assert zaim_row_data.date == '2018-11-28'
...
class TestMufgZaimRowConverterSelector:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([InstanceResource.FIXTURE_RECORD_STORE_MUFG_EMPTY],
             InstanceResource.ROW_DATA_MUFG_INCOME_CARD, MufgIncomeZaimTransferRowConverter),
            # Case when MUFG income not by card
            ([InstanceResource.FIXTURE_RECORD_STORE_MUFG_OTHER_ACCOUNT],
             InstanceResource.ROW_DATA_MUFG_INCOME_NOT_CARD, MufgZaimIncomeRowConverter),
            # Case when MUFG payment
            ([InstanceResource.FIXTURE_RECORD_STORE_MUFG_EMPTY],
             InstanceResource.ROW_DATA_MUFG_PAYMENT, MufgPaymentZaimTransferRowConverter),
            # Case when MUFG transfer income which transfer_target doesn't exist
            ([InstanceResource.FIXTURE_RECORD_STORE_MUFG_MUFG],
             InstanceResource.ROW_DATA_MUFG_TRANSFER_INCOME_NOT_OWN_ACCOUNT, MufgZaimIncomeRowConverter),
            ...
            ([InstanceResource.FIXTURE_RECORD_STORE_MUFG_MUFG_TRUST_AND_BANK],
             InstanceResource.ROW_DATA_MUFG_TRANSFER_INCOME_OWN_ACCOUNT, MufgTransferIncomeZaimTransferRowConverter),
            ...
            ([InstanceResource.FIXTURE_RECORD_STORE_MUFG_TOKYO_WATERWORKS],
             InstanceResource.ROW_DATA_MUFG_TRANSFER_PAYMENT_TOKYO_WATERWORKS, MufgZaimPaymentRowConverter),
    ...
    def test_select_factory(yaml_config_load, database_session_with_schema, input_row_data: MufgRowData, expected):
        ...
        validated_input_row = MufgRowFactory().create(AccountId.MUFG, input_row_data).validate()
        assert MufgZaimRowConverterSelector().select(validated_input_row) == expected
